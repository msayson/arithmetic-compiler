...
import parser.jcc.JCCArithLangParserConstants;
...
import static parser.jcc.JCCArithLangParserConstants.EOF;
import static parser.jcc.JCCArithLangParserConstants.IDENTIFIER;
...
public class Test2LexInternal {
    ...
    private JCCArithLangParser parserOn(String inputString) {
        return new JCCArithLangParser(new StringReader(inputString));
    }

    private JCCArithLangParser parserOn(File file) throws FileNotFoundException {
        return new JCCArithLangParser(new FileReader(file));
    }


    void test(String input, int[] tokenKinds) {
        JCCArithLangParser parser = parserOn(input);
    ...
    public void sampleCode() throws FileNotFoundException {
        ...
        for (int i = 0; i < files.length; i++) {
            JCCArithLangParser parser = parserOn(files[i]);
            System.out.println("Lexing file " + files[i]);
            Token tok;
            while ((tok = parser.getNextToken()).kind != EOF) {
                System.out.println(JCCArithLangParserConstants.tokenImage[tok.kind] + " = " + tok.image);
            }
        }
    }
